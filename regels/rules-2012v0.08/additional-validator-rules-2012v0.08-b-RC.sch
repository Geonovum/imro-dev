...
<!--
/*******************************************************************************
 * File: 	additional-validator-rules2012v0.08-b-RC.sch
 * Datum: 	17-05-2022
 * (C) Logica, 2011
 ...
 * 17-05-2021	AdB extension of validation rules for TAM + Overgangsrecht (v0.08) 
                    ** WARNING ** additional TAM-rules copied into EOF v0.07 - Schematron v0.08 does not validate succesfull:: Attribute 'name' is not allowed to appear in element 'iso:pattern'. ** WARNING **
 ******************************************************************************/
...
Geometrie waarnaar verwezen wordt bevat: Surfaces: <iso:value-of select="$begrenzingIsSurface"/>, Lines: <iso:value-of select="$begrenzingIsLine"/>, Points: <iso:value-of select="$begrenzingIsPoint"/>.
	...
	<iso:pattern id="TAM">
		...
		<iso:rule context="//imro:Bestemmingsplangebied">
			...
			<iso:assert
				test="
				if (imro:typePlan = 'bestemmingsplan' and number(replace(imro:planstatusInfo/imro:PlanstatusEnDatum/imro:datum, '-', '')) >= 20220701)
				then
				(imro:planstatusInfo/imro:PlanstatusEnDatum/imro:planstatus = 'vastgesteld' or imro:planstatusInfo/imro:PlanstatusEnDatum/imro:planstatus = 'geconsolideerd') or starts-with(imro:naam, 'TAM-omgevingsplan')
				else
				imro:planstatusInfo/imro:PlanstatusEnDatum/imro:planstatus"
			...
			<iso:assert
				test="
				if (number(replace(imro:planstatusInfo/imro:PlanstatusEnDatum/imro:datum, '-', '')) >= 20220701 and (imro:typePlan = 'inpassingsplan' or imro:typePlan = 'uitwerkingsplan' or imro:typePlan = 'wijzigingsplan'))
				then
		...
		<iso:rule context="//imro:Besluitgebied_A">
			...
			<iso:assert
				test="
				if (number(replace(imro:planstatusInfo/imro:PlanstatusEnDatum/imro:datum, '-', '')) >= 20220701) then
				imro:typePlan != 'amvb' and imro:typePlan != 'regeling'
				else
				imro:typePlan"
				> IMRO-object met gml:id <iso:value-of select="@gml:id"/>, type = <iso:value-of
					select="name()"/>: Fout in typePlan -> Als imro:datum is groter dan of gelijk
				aan 2022-07-01, dan mag typePlan niet zijn 'amvb' of 'regeling'. 
			</iso:assert>
		</iso:rule>
		...
		<iso:rule context="//imro:Besluitgebied_P">
			...
			<iso:assert
				test="
				if (number(replace(imro:planstatusInfo/imro:PlanstatusEnDatum/imro:datum, '-', '')) >= 20220701)
				then
				imro:typePlan != 'provinciale verordening'
				else
				imro:typePlan"
				> IMRO-object met gml:id <iso:value-of select="@gml:id"/>, type = <iso:value-of
					select="name()"/>: Fout in typePlan -> Als imro:datum is groter dan of gelijk
				aan 2022-07-01, dan mag typePlan niet zijn 'provinciale verordening'. 
			</iso:assert>
		</iso:rule>
		...
		<iso:rule context="//imro:Besluitgebied_X">
			...
			<iso:assert
				...
				if (number(replace(imro:planstatusInfo/imro:PlanstatusEnDatum/imro:datum, '-', '')) >= 20220701)
				then
				imro:typePlan != 'aanwijzingsbesluit' and imro:typePlan != 'beheersverordening' and imro:typePlan != 'reactieve aanwijzing'
				else
				imro:typePlan"
				> IMRO-object met gml:id <iso:value-of select="@gml:id"/>, type = <iso:value-of
					select="name()"/>: Fout in typePlan -> Als imro:datum is groter dan of gelijk
			...
			<iso:assert
				...
				if (number(replace(imro:planstatusInfo/imro:PlanstatusEnDatum/imro:datum, '-', '')) >= 20220701)
				then
				imro:typePlan != 'voorbereidingsbesluit' or starts-with(imro:naam, 'TAM')
				else
				imro:typePlan"
				> IMRO-object met gml:id <iso:value-of select="@gml:id"/>, type = <iso:value-of
					select="name()"/>: Fout in typePlan -> Als imro:datum is groter dan of gelijk
			...
			<iso:assert
				test="
				if (number(replace(imro:planstatusInfo/imro:PlanstatusEnDatum/imro:datum, '-', '')) >= 20220701 and imro:typePlan = 'exploitatieplan')
				then
				imro:planstatusInfo/imro:PlanstatusEnDatum/imro:planstatus != 'ontwerp'
				else
				imro:planstatusInfo/imro:PlanstatusEnDatum/imro:planstatus"
		...
		<iso:rule context="//imro2008:Besluitgebied_X">
			...
			<iso:assert
				test="
				if (number(replace(imro2008:planstatusInfo/imro2008:PlanstatusEnDatum_XGB/imro2008:datum, '-', '')) >= 20220701)
				then
				imro2008:typePlan != 'projectbesluit' and imro2008:typePlan != 'tijdelijke ontheffing buitenplans'
				else
				imro2008:typePlan"
				> IMRO-object met gml:id <iso:value-of select="@gml:id"/>, type = <iso:value-of
					select="name()"/>: Fout in typePlan -> Als imro:datum is groter dan of gelijk
