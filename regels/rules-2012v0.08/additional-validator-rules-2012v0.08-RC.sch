...
<iso:schema xmlns:iso="http://purl.oclc.org/dsdl/schematron"
	...
	<iso:let name="bestandsnaamRegexp"
		value="concat($prefix, $idncheck, $naamverlenging, $alleToegestaneExtensies, $optioneleSuffix)"/>

	<iso:let name="illustratieExtensies" value="'\.(jpg|jpeg|png|pdf)'"/>
	<iso:let name="illustratieBestandsnaamRegexp"
		value="concat('i_', $idncheck, $naamverlenging, $illustratieExtensies, $optioneleSuffix)"/>

	<iso:let name="bijlageExtensies" value="'\.(htm|html|xhtml|pdf)'"/>
	<iso:let name="bijlageBestandsnaamRegexp"
		value="concat('b_', $idncheck, $naamverlenging, $bijlageExtensies, $optioneleSuffix)"/>

	<iso:let name="vaststellingsbesluitExtensies" value="'\.(htm|html|xhtml|pdf)'"/>
	<iso:let name="vaststellingsbesluitBestandsnaamRegexp"
		value="concat('vb_', $idncheck, $vaststellingsbesluitExtensies, $optioneleSuffix)"/>

	<iso:let name="regelExtensies" value="'\.(htm|html|xhtml)'"/>
	<iso:let name="regelBestandsnaamRegexp"
		value="concat('r_', $idncheck, $regelExtensies, $optioneleSuffix)"/>

	<iso:let name="toelichtingExtensies" value="'\.(htm|html|xhtml|pdf)'"/>
	<iso:let name="toelichtingBestandsnaamRegexp"
		value="concat('t_', $idncheck, $toelichtingExtensies, $optioneleSuffix)"/>

	<iso:let name="beleidBesluitExtensies" value="'\.(htm|html|xhtml|pdf)'"/>
	<iso:let name="beleidBesluitBestandsnaamRegexp"
		value="concat('d_', $idncheck, $beleidBesluitExtensies, $optioneleSuffix)"/>

	<iso:let name="plantextExtensies" value="'\.xml'"/>
	<iso:let name="plantextBestandsnaamRegexp"
		value="concat('pt_', $idncheck, $plantextExtensies, $optioneleSuffix)"/>
	...
	<iso:let name="containsVerwijzingenNaarObjectgerichteTekst"
		value="boolean(//imro:verwijzingNaarTekst[starts-with(text(), 'pt_')])"/>
	...
	<xsl:key name="GeometrieStructuurvisieObject_P_index"
		match="//imro:GeometrieStructuurvisieObject_P" use="@gml:id"/>
	<xsl:key name="GeometrieStructuurvisieObject_index" match="//imro:GeometrieStructuurvisieObject"
		use="@gml:id"/>
	<xsl:key name="GeometrieBesluitobject_A_index" match="//imro:GeometrieBesluitobject_A"
		use="@gml:id"/>
	<xsl:key name="GeometrieBesluitobject_P1_index" match="//imro:GeometrieBesluitobject_P1"
		use="@gml:id"/>
	<xsl:key name="GeometrieBesluitobject_P3_index" match="//imro:GeometrieBesluitobject_P3"
		use="@gml:id"/>
	...
	<xsl:key name="VVPCartografischeInfoMetValideSymboolCodeByGmlIdKey"
		match="//imro:cartografieInfo/imro:CartografieInfo[regexp:matches(imro:symboolcode, $VVPSymboolcodeRegexp)]"
		use="ancestor::imro:*[@gml:id]/@gml:id"/>
	...
	<xsl:key name="vvpBegrenzingPerGeometrieTypeSurface"
		match="imro:begrenzing[@xlink:href and key('surface_index', substring-after(@xlink:href, '#'))]"
		use="substring-after(@xlink:href, '#')"/>
	<xsl:key name="vvpBegrenzingPerGeometrieTypeLine"
		match="imro:begrenzing[@xlink:href and key('line_index', substring-after(@xlink:href, '#'))]"
		use="substring-after(@xlink:href, '#')"/>
	<xsl:key name="vvpBegrenzingPerGeometrieTypePoint"
		match="imro:begrenzing[@xlink:href and key('point_index', substring-after(@xlink:href, '#'))]"
		use="substring-after(@xlink:href, '#')"/>
	...
	<iso:pattern name="Algemene_checks">
		...
		<iso:rule abstract="true" id="check_naam_not_empty">
			...
			<iso:assert test="regexp:matches(imro:naam, '[^ \t\r\n]{1}.*')"
				diagnostics="identificatie"> (Planobject gml:id = <iso:value-of
					select="$identificatie"/>, type = <iso:value-of select="name()"/>) Foutcode
				AD80: Het element imro:naam (type gml:CodeType) van een plangebied of planobject mag
				niet leeg zijn of met een spatie, tab, return of new line beginnen. </iso:assert>
		</iso:rule>
		...
		Deze test is afgeleid uit paragraaf 11.2 van IMRO2012.pdf.
		 -->
			...
			<iso:assert test="$identificatie = $composedID" diagnostics="identificatie"> (Planobject
				gml:id = <iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name()"/>) Foutcode AD0A: Object planobject: De waarde voor gml:id
				(huidige waarde: '<iso:value-of select="$identificatie"/>') is opgebouwd uit de
				waarden van het samengestelde attribuut imro:identificatie volgens het format gml:id
				= namespace '.' lokaalID (huidige waarde: '<iso:value-of select="$composedID"/>').
				Zie hiervoor IMRO2012.pdf paragraaf 11.2 "Nadere implementatie specificaties". </iso:assert>
		...
		Deze test is afgeleid uit paragraaf 11.2 van IMRO2012.pdf.
		 -->
			<iso:assert test="$identificatie = $composedID" diagnostics="identificatie"> (Planobject
				gml:id = <iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name()"/>) Foutcode AD0B: Object plangebied: De waarde voor gml:id
				(huidige waarde: '<iso:value-of select="$identificatie"/>') is opgebouwd uit de
				waarden van het samengestelde attribuut imro:identificatie volgens het format gml:id
				= namespace '.' lokaalID '-' versie (huidige waarde: '<iso:value-of
					select="$composedID"/>'). Zie hiervoor IMRO2012.pdf paragraaf 11.2 "Nadere
				implementatie specificaties". </iso:assert>

			<iso:assert test="regexp:matches(imro:overheidsCode, $overheidsCodeFormat)"
				diagnostics="identificatie"> (Planobject gml:id = <iso:value-of
					select="$identificatie"/>, type = <iso:value-of select="name()"/>) Foutcode AD1:
				overheidsCode (huidige waarde "<iso:value-of select="imro:overheidsCode"/>") moet
				uit 4 cijfers bestaan. Zie hiervoor o.a. paragraaf 6.4.1 (afhankelijk van
				planobject) in IMRO2012.pdf. </iso:assert>

			<iso:assert test="regexp:matches($identificatie, $idncheck)" diagnostics="identificatie"
				> (Planobject gml:id = <iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name()"/>) Foutcode AD2: gml:id (huidige waarde "<iso:value-of
					select="$identificatie"/>") voldoet niet aan de conventies van STRI2012. </iso:assert>

			<iso:assert test="regexp:matches($lokaalID, $lokaalIDFormat)"> Foutcode AD3:
				NEN3610ID.lokaalID (huidige waarde: <iso:value-of select="$lokaalID"/>) voldoet niet
				aan het gespecificeerde format '[0-9]{4}\.[A-Za-z0-9]{1,18}' Zie constraint a4 van
				IMRO2012.pdf paragraaf 8.4. </iso:assert>
		</iso:rule>
	...
	<iso:pattern name="SVBegrenzingVerwijzingenBestaan">
		...
		<iso:rule abstract="true" id="check_begrenzingen">
			...
			<iso:assert test="string-length(@xlink:href) &gt; 0" diagnostics="identificatie">
				(Planobject gml:id = <iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name($ownerFeature)"/>) Foutcode AD30A: <iso:value-of
					select="name($ownerFeature)"/>/imro:begrenzing dient een gevuld xlink:href
				attribuut te bevatten. </iso:assert>
			...
			<iso:assert test="key($begrenzingIndex, $begrenzing_id)" diagnostics="identificatie">
				(Planobject gml:id = <iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name($ownerFeature)"/>) Foutcode AD30B: <iso:value-of
					select="name($ownerFeature)"/> bevat begrenzing zonder valide verwijzing
				(huidige waarde : <iso:value-of select="$begrenzing_href"/>) naar <iso:value-of
					select="$begrenzingObjectType"/>
			</iso:assert>
		</iso:rule>


		<iso:rule context="
				//imro:Structuurvisieplangebied_G/imro:begrenzing |
		...
		<iso:rule context="
				//imro:Structuurvisieverklaring_P/imro:begrenzing |
		...
		<iso:rule context="
				//imro:Structuurvisiecomplex_G/imro:begrenzing |
	...
	<iso:pattern name="GBBegrenzingVerwijzingenBestaan">
		...
		<iso:rule context="
				//imro:Besluitsubvlak_A/imro:begrenzing |
		...
		<iso:rule context="
				//imro:Besluitgebied_A/imro:begrenzing |
	...
	<iso:pattern name="gml_checks">
		...
		<iso:rule context="//gml:CircleByCenterPoint">
			...
			<iso:assert test="false" diagnostics="identificatie"> (Planobject gml:id = <iso:value-of
					select="$identificatie"/>, type = <iso:value-of select="name($ownerFeature)"/>)
				Foutcode AD60: Gebruik van GML element <iso:name/> is niet toegestaan. Zie paragraaf
				11.2 (Gedeelte "Geometrietypen en interpolatie" uit hoofdstuk 11 "GML
				Specificaties") uit IMRO2012.pdf. </iso:assert>
		</iso:rule>
		...
		<iso:rule context="//gml:posList | //gml:pos | //gml:lowerCorner | //gml:upperCorner">
			...
			<iso:let name="ownerFeature"
				value="ancestor::imro:featureMember/imro:* | ancestor::gml:boundedBy"/>
			<iso:let name="identificatie" value="$ownerFeature/@gml:id"/>
			...
			<iso:assert test="regexp:matchesAfterTokenize($coordinates, $coordinaatFormat, $space)"
				diagnostics="coords identificatie"> (Planobject gml:id = <iso:value-of
					select="$identificatie"/>, type = <iso:value-of select="name($ownerFeature)"/>)
				Foutcode AD61A: coordinaten binnen <iso:value-of select="name()"/> moeten getallen
				zijn van maximaal 3 decimalen na de decimale komma, waarbij meerdere exemplaren
				gescheiden zijn door lege ruimten. Afwijking binnen element <iso:value-of
					select="ancestor::*[@gml:id][1]/@gml:id"/>. Eerste coordinaten zijn:
					<iso:value-of select="$coords"/>
			</iso:assert>

			<iso:let name="aantal" value="regexp:tokenCount($coordinates, $space)"/>

			<iso:assert test="(2 * floor($aantal div 2) = $aantal) and ($aantal > 0)"
				diagnostics="coords identificatie"> (Planobject gml:id = <iso:value-of
					select="$identificatie"/>, type = <iso:value-of select="name($ownerFeature)"/>)
				Foutcode AD61B: coordinaten binnen <iso:value-of select="name()"/> komen niet in het
				juiste aantal voor. Verwacht wordt dat dit een even aantal coordinaten ( &gt; 0 )
				is, geconstateerd aantal is <iso:value-of select="$aantal"/>. Afwijking binnen
				element <iso:value-of select="ancestor::*[@gml:id][1]/@gml:id"/>. Eerste coordinaten
				zijn: <iso:value-of select="$coords"/>
			</iso:assert>
		</iso:rule>
		...
		<iso:rule context="//*[@srsName]">
			...
			<iso:assert test="@srsName = 'urn:ogc:def:crs:EPSG::28992'" diagnostics="identificatie">
				(Planobject gml:id = <iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name($ownerFeature)"/>) Foutcode AD62A: srsName (huidige waarde
					"<iso:value-of select="@srsName"/>") moet gelijk zijn aan
				urn:ogc:def:crs:EPSG::28992. </iso:assert>
		</iso:rule>
		...
		<iso:rule context="//gml:posList | //gml:pos | //gml:lowerCorner | //gml:upperCorner">
			...
			<iso:assert test="ancestor::*[@srsName]" diagnostics="identificatie coordsLookup">
				(Planobject gml:id = <iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name($ownerFeature)"/>) Foutcode AD62B: Geen srsName aanwezig voor
				geometrisch element <iso:value-of select="name()"/>. Eerste coordinaten zijn:
					<iso:value-of select="substring(text(), 1, 66)"/>
			</iso:assert>
		</iso:rule>
	 ...
	 -->
		<iso:rule context="
			...
			<iso:let name="oneOfPermitted"
				value="contains('pos,posList,lowerCorner,upperCorner', local-name())"/>
			...
			<iso:assert test="$otherLowestGmlType or $oneOfPermitted" diagnostics="identificatie">
				(Planobject gml:id = <iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name($ownerFeature)"/>) Foutcode AD70: <iso:value-of select="name()"/>
				is een deel van een geometrie zonder coordinaten inhoud. </iso:assert>
		</iso:rule>
	...
	<iso:pattern name="Bestandsnaamconventies">
		...
		<iso:rule abstract="true" id="algemene_bestandsnaam_check">
			...
			<iso:let name="isAlgemeenValide"
				value="regexp:matches($verwijzing, $bestandsnaamRegexp)"/>

			<iso:assert test="$isAlgemeenValide" diagnostics="identificatie"> (Planobject gml:id =
					<iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name($ownerFeature)"/>) Foutcode AD5A: <iso:value-of select="name()"/>
				(huidige waarde "<iso:value-of select="text()"/>") is een bestandsnaam die niet aan
				de reguliere expressie van de bestandsnaamconventies van STRI2012 voldoet. </iso:assert>
		...
		<iso:rule context="//imro:verwijzingNaarVaststellingsbesluit">
			...
			<iso:assert test="
					not($isAlgemeenValide) or
					regexp:matches($verwijzing, $vaststellingsbesluitBestandsnaamRegexp)"
				diagnostics="identificatie"> (Planobject gml:id = <iso:value-of
					select="$identificatie"/>, type = <iso:value-of select="name($ownerFeature)"/>)
				Foutcode AD5B: <iso:value-of select="name()"/> (huidige waarde "<iso:value-of
					select="$verwijzing"/>") is een bestandsnaam die niet aan de reguliere expressie
				van de bestandsnaamconventies van STRI2012 voldoet. </iso:assert>
		</iso:rule>
		...
		<iso:rule context="//imro:IllustratieReferentie/imro:verwijzingNaarIllustratie">
			...
			<iso:assert test="
					not($isAlgemeenValide) or
					regexp:matches($verwijzing, $illustratieBestandsnaamRegexp)"
				diagnostics="identificatie"> (Planobject gml:id = <iso:value-of
					select="$identificatie"/>, type = <iso:value-of select="name($ownerFeature)"/>)
				Foutcode AD5B: <iso:value-of select="name()"/> (huidige waarde "<iso:value-of
					select="$verwijzing"/>") is een bestandsnaam die niet aan de reguliere expressie
		...
		<iso:rule context="//imro:verwijzingNaarTekstInfo/imro:*/imro:verwijzingNaarTekst">
			...
			<iso:assert test="
					...
					($isPlanTekst and regexp:matches($verwijzing, $plantextBestandsnaamRegexp))"
				diagnostics="identificatie"> (Planobject gml:id = <iso:value-of
					select="$identificatie"/>, type = <iso:value-of select="name($ownerFeature)"/>)
				Foutcode AD5B: <iso:value-of select="name()"/> (huidige waarde "<iso:value-of
					select="$verwijzing"/>") is een bestandsnaam die niet aan de reguliere expressie
			...
			<iso:let name="isRegelToelichtingBeleidBesluit"
				value="($isBeleidBesluit or $isToelichting or $isRegel)"/>
			...
			Dit betekent dat de combinatie regel/toelichting/beleid/besluit document en verwijzing naar objectgerichte teksten verboden is.
			 -->
			<iso:assert
				test="not($isRegelToelichtingBeleidBesluit and $containsVerwijzingenNaarObjectgerichteTekst)"
				> Foutcode AD5C: <iso:value-of select="name()"/> (huidige waarde "<iso:value-of
					select="$verwijzing"/>") is een verwijzing naar tekst met een onverwachte
				prefix. Indien er verwijzingen naar tekst voorkomen met prefix 'pt_' dan is de
				prefix 'r_', 't_' of 'd_' niet toegestaan en omgekeerd. </iso:assert>
...
-->
			...
			<iso:let name="naamAfterHoofdgroep"
				value="substring($naam, string-length($hoofdgroep) + 1)"/>

			<iso:let name="naamEndRegexp" value="' - .*'"/>

			<iso:assert test="
					$hoofdgroepIsOverig or
					($naamLowercase = $hoofdgroep) or
					(starts-with($naamLowercase, $hoofdgroep) and regexp:matches($naamAfterHoofdgroep, $naamEndRegexp))"
				diagnostics="identificatie"> Foutcode AD14: De naam van de <iso:value-of
					select="name()"/> '<iso:value-of select="$identificatie"/>' (huidige waarde:
					'<iso:value-of select="$naam"/>') voldoet niet aan SVBP2012 paragraaf 3.3 waarin
				staat dat deze begint met de hoofdgroep (huidige waarde ongelijk aan 'overige':
					'<iso:value-of select="$hoofdgroep"/>') eventueel gevolgd door een koppelteken
				('-') met daarachter de rest van de naam. </iso:assert>
		</iso:rule>
	</iso:pattern>
	...
	<iso:pattern name="CheckGebiedsaanduidinggroepNaamMatch">
		...
		<iso:rule context="//imro:Gebiedsaanduiding">
			...
			<iso:assert test="
					(starts-with($gebiedsaanduidingnaam, 'other:') and ($gebiedsaanduidinggroep = 'overig'))
					or
					(starts-with($gebiedsaanduidingnaam, $gebiedsaanduidinggroep))"
				diagnostics="identificatie"> Foutcode AD10: In gebiedsaanduiding '<iso:value-of
					select="@gml:id"/>' komt categorie imro:gebiedsaanduidinggroep (huidige
					waarde='<iso:value-of select="$gebiedsaanduidinggroep"/>') niet overeen met
				imro:naam (huidige waarde='<iso:value-of select="$gebiedsaanduidingnaam"/>') volgens
				SVBP2012 </iso:assert>
		</iso:rule>
	</iso:pattern>
...
Rules AD21A and AD21B removed.
 -->
	 ...
	 -->
			<iso:let name="isValideSymboolcode"
				value="not(imro:symboolcode) or regexp:matches(imro:symboolcode, $VVPSymboolcodeRegexp)"/>

			<iso:assert test="$isValideSymboolcode" diagnostics="identificatie"> (Planobject gml:id
				= <iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name($ownerFeature)"/>) Foutcode AD20: Element met identificatie
					<iso:value-of select="$identificatie"/> met cartografieInfo heeft een
				symboolcode (huidige waarde '<iso:value-of select="imro:symboolcode"/>'). Deze moet
				beginnen met 'ES', 'AS', 'S', 'L' of 'P' met daarachter een nummer. </iso:assert>

			<!-- Check het kaartnummer
	 -->
			<iso:let name="isValideKaartnummer" value="regexp:matches(imro:kaartnummer, '\d+')"/>

			<iso:assert test="$isValideKaartnummer" diagnostics="identificatie"> (Planobject gml:id
				= <iso:value-of select="$identificatie"/>, type = <iso:value-of
					select="name($ownerFeature)"/>) Foutcode AD20: Element met identificatie
					<iso:value-of select="$identificatie"/> met cartografieInfo heeft een
				kaartnummer (huidige waarde '<iso:value-of select="imro:kaartnummer"/>'). Dit veld
				dient een numerieke waarde te hebben. </iso:assert>
			...
			<iso:let name="cartografischeInfosMetAndereKaartnaamBijNummer"
				value="key('VVPKaartNummersKey', $kaartnummer)[not(imro:kaartnaam = $kaartnaam)]"/>
			...
			<!--
			 ...
			 -->
			<iso:assert test="count($cartografischeInfosMetAndereKaartnaamBijNummer) = 0"
				diagnostics="identificatie"> (Planobject gml:id = <iso:value-of
					select="$identificatie"/>, type = <iso:value-of select="name($ownerFeature)"/>)
				Foutcode AD23: Element met identificatie <iso:value-of
					select="ancestor::*[@gml:id][1]/@gml:id"/> bevat een kaartnummer-kaartnaam
				combinatie die niet uniek is wegens dat er reeds andere combinaties voorkomen.
				Huidig kaartnummer='<iso:value-of select="$kaartnummer"/>'. Andere kaartnamen:
					<iso:value-of
					select="regexp:string-join($cartografischeInfosMetAndereKaartnaamBijNummer/imro:kaartnaam, ' , ')"
				/>. Bij een kaartnummer hoort slechts 1 kaartnaam. </iso:assert>
		...
		<!-- Check de geometrie van de begrenzingen met de cartografischeInfo's
 		...
 		-->
		<iso:rule
			context="imro:begrenzing[key('VVPCartografischeInfoMetValideSymboolCodeByGmlIdKey', parent::imro:*[@gml:id]/@gml:id) and @xlink:href]">
			...
			<iso:let name="begrenzingIsSurface"
				value="boolean(key('vvpBegrenzingPerGeometrieTypeSurface', $objectId))"/>
			<iso:let name="begrenzingIsLine"
				value="boolean(key('vvpBegrenzingPerGeometrieTypeLine', $objectId))"/>
			<iso:let name="begrenzingIsPoint"
				value="boolean(key('vvpBegrenzingPerGeometrieTypePoint', $objectId))"/>
			...
			<iso:let name="geometrieKomtOvereenMetCartografischeInfo" value="
					(not($begrenzingIsSurface) or $cartografischeInfosVoorGeometrieSurface) and
			...
			<iso:assert test="$geometrieKomtOvereenMetCartografischeInfo"
				diagnostics="identificatie"> (Planobject gml:id = <iso:value-of
					select="$identificatie"/>, type = <iso:value-of select="name($ownerFeature)"/>)
				Foutcode AD25: Element <iso:value-of select="name(parent::imro:*)"/> bevat een
				begrenzing (xlink:href="<iso:value-of select="$objectId"/>") die naar een object met
				een geometrie verwijst die niet voorkomt in de symboolcodes van de aanwezige
				cartografischeInfo (huidige waarde: "<iso:value-of
					select="regexp:string-join(../imro:cartografieInfo/imro:CartografieInfo/imro:symboolcode, ' , ')"
				/>"). Geometrie waarnaar verwezen wordt bevat: Surfaces: <iso:value-of
					select="$begrenzingIsSurface"/>, Lines: <iso:value-of select="$begrenzingIsLine"
				/>, Points: <iso:value-of select="$begrenzingIsPoint"/>. </iso:assert>
	...
	<iso:pattern name="overige_planobjecten_en_plangebieden">
		...
		<iso:rule context="//imro:*[imro:typePlan]">
			...
			<iso:let name="beleidsmatigVerantwoordelijkeOverheid"
				value="//*[imro:beleidsmatigVerantwoordelijkeOverheid]/imro:beleidsmatigVerantwoordelijkeOverheid"/>
			<iso:let name="isAanwijzingsbesluit" value="$detectedPlanType = 'aanwijzingsbesluit'"/>
			<iso:let name="isGemeentelijk"
				value="$beleidsmatigVerantwoordelijkeOverheid = 'gemeentelijke overheid'"/>

			<!-- assertions: -->
			<iso:assert test="not($isAanwijzingsbesluit and $isGemeentelijk)"> (Planobject gml:id =
					<iso:value-of select="$identificatie"/>) Foutcode AD26: Het plantype
				aanwijzingsbesluit kan niet door een Gemeentelijke overheid gepubliceerd worden. </iso:assert>
	...
	<iso:pattern id="TAMomgevingsplan">
		<iso:rule context="//imro:Bestemmingsplangebied[//imro:typePlan = 'bestemmingsplan']">
			...
			<iso:assert test="
					...
					select="name()"/>: Fout in planstatus -> Als typePlan is 'bestemmingsplan' en
				naam begint met 'TAM-omgevingsplan ', dan moet planstatus zijn 'concept' of
				'voorontwerp' of 'ontwerp'.</iso:assert>
		</iso:rule>
	</iso:pattern>
	...
	<iso:diagnostics>
		...
		<iso:diagnostic id="identificatieLookup"><iso:value-of select="ancestor::imro:*/@gml:id"
			/></iso:diagnostic>

		<iso:diagnostic id="coords"><iso:value-of select="$coords"/></iso:diagnostic>
		<iso:diagnostic id="coordsLookup"><iso:value-of
				select="substring(normalize-space(text()), 1, 66)"/></iso:diagnostic>

		<iso:diagnostic id="warning"/>
	</iso:diagnostics>
