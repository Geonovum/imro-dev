...
<!-- xmlns:fn="http://www.w3.org/2004/07/xpath-functions" -->
 ...
 <!-- <sch:schema xmlns:sch="http://www.ascc.net/xml/schematron"> Schematron namespace -->
	...
	<iso:ns prefix="regexp" uri="urn:java:nl.vrom.roo.util.Regexp"/>
...
Geometrie waarnaar verwezen wordt bevat: Surfaces: <iso:value-of select="$begrenzingIsSurface"/>, Lines: <iso:value-of select="$begrenzingIsLine"/>, Points: <iso:value-of select="$begrenzingIsPoint"/>.
	...
	<iso:pattern name="overige_planobjecten_en_plangebieden">
		...
		<iso:rule context="//imro:*[imro:typePlan]">
			...
			<!-- Thijs Brentjens : check if the typePlan and overheid are okay -->
			<!-- Thijs Brentjens: TODO: check if this works -->
			...
			<!-- Thijs: end modifications -->
			<!-- assertions: -->
			<iso:assert test="not(@beleidsmatigVerantwoordelijkeOverheid = 'gemeentelijke overheid' and @detectedPlanType = 'aanwijzingsbesluit')">
				<!-- Thijs: TODO: error message, foutcode -->
(Planobject gml:id = <iso:value-of select="$identificatie"/>)
Foutcode XXXXX : Het plantype ‘aanwijzingsbesluit’ kan niet door een Gemeentelijk overheid gepubliceerd worden.
		   </iso:assert>
